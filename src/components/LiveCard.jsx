...
const LiveCard = ({ media, onClick }) => {
  ...
  useEffect(() => {
      ...
      if (!logoUrl) {
        try {
          const wr = await fetch(
            `https://en.wikipedia.org/w/api.php?action=query&prop=pageimages&titles=${encodeURIComponent(teamName)}&pithumbsize=100&format=json&origin=*`,
          );
          if (wr.ok) {
    ...
    if (media.teams.home?.name)
      resolveTeamLogo(media.teams.home.name, setHomeLogo);
    if (media.teams.away?.name)
      resolveTeamLogo(media.teams.away.name, setAwayLogo);
  }, [media]);
        ...
        <div className="lc-teams">
          <div className="lc-team">
            ...
            <div
              ...
              style={{
                display: homeLogo || initialHomeBadge ? "none" : "flex",
              }}
            >
              X
            </div>
          ...
          <div className="lc-team">
            ...
            <div
              className="fallback-logo"
              style={{
                display: awayLogo || initialAwayBadge ? "none" : "flex",
              }}
            >
              X
            </div>
  ...
  const parsePoster = (posterUrl) => {
    ...
    if (posterUrl.startsWith("http"))
      return `https://wsrv.nl/?url=${encodeURIComponent(posterUrl)}`;
    if (posterUrl.startsWith("/"))
      return `https://wsrv.nl/?url=${encodeURIComponent("https://streamex.sh" + posterUrl)}`;
    return "";
  };
  ...
  return (
    <div className="live-card-v2" onClick={handleClick}>
      ...
      <div className="lc-footer" style={{ borderTop: "none" }}>
        {title}
      </div>
    </div>
  );
