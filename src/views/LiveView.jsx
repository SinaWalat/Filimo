    ...
    <section className="movie-section" style={{ marginTop: "1rem" }}>
      ...
      >
        <i
          ...
          style={{ color: "var(--accent-primary)", fontSize: "1.5rem" }}
        ></i>
        <h1 style={{ fontSize: "2rem" }}>Live Sports</h1>
      </div>
      <p style={{ color: "var(--text-secondary)", marginBottom: "2rem" }}>
        Watch live sports events from around the world
      </p>

      <div className="live-filters">
        <div className="lf-left">
          <button
            className={`lf-btn ${timeFilter === "live" ? "active" : ""}`}
            onClick={() => setTimeFilter("live")}
          >
            <i className="fa-solid fa-broadcast-tower"></i> Live Now
          </button>
          <button
            className={`lf-btn ${timeFilter === "today" ? "active" : ""}`}
            onClick={() => setTimeFilter("today")}
          >
            <i className="fa-regular fa-calendar"></i> Today
          </button>
          <button
            className={`lf-btn ${timeFilter === "all" ? "active" : ""}`}
          ...
          >
            ...
            <div className={`cs-dropdown ${isDropdownActive ? "active" : ""}`}>
              <div
                className={`cs-option ${catFilter === "all" ? "selected" : ""}`}
              ...
              {Object.keys(categories)
                ...
                .map((cat) => (
                  <div
                    key={cat}
                    className={`cs-option ${catFilter === cat ? "selected" : ""}`}
        ...
        {!loading &&
          ...
          Object.entries(categories).map(([catName, items]) => {
            ...
            return (
              <div
                key={catName}
                className="live-category-section"
